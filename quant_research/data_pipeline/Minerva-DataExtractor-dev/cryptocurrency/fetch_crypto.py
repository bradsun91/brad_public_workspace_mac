...
class Crypto(object):
    ...
    def get_crypto(self):
        ...
        data = yahoo.get_historical_price_data(start_date=self.yesterday,end_date=self.yesterday, time_interval='daily')
        ...
        for ticker in self.tickers:
            df = {}
            df['ticker'] = ticker
            df.update(data[ticker]['prices'][0])
            ...
            with open('crypto_data/{}_crypto_data.json'.format(self.yesterday), 'a') as outfile:
                outfile.write(df)
