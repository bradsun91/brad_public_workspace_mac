...
def down_stk010(rdat,xcod,xtyp,fgInx=False):
    ...
    try:
        ...
        #----------
        #xdk=ts.get_hist_data(xcod, start=tim0, end=None,ktype=xtyp)
...
def down_tick010(xcod,xtim,ftg):
    ...
    #vlst=['sn','tt','nt']
    #serv=random.choice(vlst);print('v',serv)
    serv='sn'
    try:
        #df = ts.get_tick_data(xcod,date=xtim)    #print(df.head())
        df = ts.get_tick_data(xcod,date=xtim,src=serv)    #print(df.head())
...
def down_tickLib8tim_pools(rs0,stkPool,xtim0,xtim9):
    '''
    下载股票池，所有股票代码的所有tick历史分笔数据，按时间日期循环下载
    数据文件保存在：对应的数据目录 \zwdat\tick\yyyy-mm\
        目录下，yyyy，是年份；mm，是月份
    [输入]
      rs0,保存tick数据的目录
