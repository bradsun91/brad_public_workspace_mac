...
def ADX(df, n, n_ADX,ksgn='close'):
    ...
    ATR = TR_s.ewm(span=n).mean()
    UpI = pd.Series(UpI)
    DoI = pd.Series(DoI)
    PosDI =UpI.ewm(span=n).mean()/ ATR
    NegDI =DoI.ewm(span=n).mean()/ ATR
    #PosDI = pd.Series(pd.ewma(UpI, span=n, min_periods=n - 1) / ATR)
    #NegDI = pd.Series(pd.ewma(DoI, span=n, min_periods=n - 1) / ATR)
    #
    ds=pd.Series(abs(PosDI - NegDI) / (PosDI + NegDI))
    ds2=ds.ewm(span=n_ADX).mean()
    #ds2 = pd.Series(pd.ewma(abs(PosDI - NegDI) / (PosDI + NegDI), span=n_ADX, min_periods=n_ADX - 1), name=xnam)
    #ds2 = pd.Series(pd.ewma(abs(PosDI - NegDI) / (PosDI + NegDI), span=n_ADX, min_periods=n_ADX - 1), name=xnam)
    ds2.index=df.index;df[xnam]=ds2
    #df = df.join(ds)  
    return df
...
def ATR(df, n,ksgn='close'):  
    ...
    # for i, idx in enumerate(df.index)
        ...
        i = i + 1;#print('#',i,TR)
    TR_s = pd.Series(TR_l)
    ds2 = TR_s.ewm(span=n).mean()
    #ds2 = pd.Series(pd.ewma(TR_s, span=n, min_periods=n), name=xnam)
    #df = df.join(ds)  
    ds2.index=df.index;df[xnam]=ds2
    #print('ds',ds.head())
    return df
...
def CCI(df, n,ksgn='close'):  
    ...
    CCI = pd.Series((PP - PP.rolling(window=n,center=False).mean()) /PP.rolling(window=n,center=False).std(), name = xnam)  
    df = df.join(CCI)  
...
def KST(df, r1, r2, r3, r4, n1, n2, n3, n4,ksgn='close'): 
    ...
    KST = pd.Series(ROC1.rolling(window=n1,center=False).sum() + ROC2.rolling(window=n2,center=False).sum() * 2 
                    +ROC4.rolling(window=n4,center=False).sum()*3 +ROC4.rolling(window=n4,center=False).sum() * 4 , name = xnam)  
    df = df.join(KST)  
    return df
...
def MACD(df, n_fast, n_slow,ksgn='close'): 
    ...
    【输出】    
        ...
        #pd.Series(data['close']).ewm(span=short).mean()
    ...
    EMAfast=df[ksgn].ewm(span = n_fast).mean()
    EMAslow=df[ksgn].ewm(span = n_slow).mean()
    #EMAfast=ds.rolling(center=False,window=n_slow - 1).mean()
    #EMAfast=df[ksgn].ewm(span = n_fast).rolling(center=False,window=n_slow - 1).mean()
    #EMAslow = df[ksgn].ewm(span =n_slow).rolling(center=False,window=n_slow - 1).mean()
    #ds=pd.Series(df[ksgn].ewm(span = n_slow))
    #EMAslow=ds.rolling(center=False,window=n_slow - 1).mean()
    
    #
    MACD = pd.Series(EMAfast - EMAslow, name = xnam)  
    #MACDsign = pd.Series(pd.ewma(MACD, span = 9, min_periods = 8), name =xnam2)  
    MACDsign = pd.Series(MACD.ewm(span = 9).mean(), name =xnam2)  
...
　　MASS指标是所有区间震荡指标中，风险系数最小的一个。		
    ...
    #
    EX1 = Range.ewm(span = 9).mean()
    EX2 = EX1.ewm(span = 9).mean()
    #EX1 = pd.ewma(Range, span = 9, min_periods = 8)  
    #EX2 = pd.ewma(EX1, span = 9, min_periods = 8)  
    Mass = EX1 / EX2  
    #MassI = pd.Series(pd.rolling_sum(Mass, 25), name = xnam)  #'Mass Index'
    MassI = pd.Series(Mass.rolling(window=25,center=False).sum(), name = xnam)  #'Mass Index'
    df = df.join(MassI)  
    return df    
...
def RSI(df, n):
    ...
    DoI = pd.Series(DoI)
    #
    PosDI = UpI.ewm( span=n).mean()
    NegDI = DoI.ewm( span=n).mean()
...
def STDDEV(df, n,ksgn='close'):
    ...
    #
    ds=df[ksgn].rolling(window=n,center=False).std()
    #df = df.join(pd.Series(pd.rolling_std(df[ksgn], n), name =xnam))  
    df = df.join(pd.Series(ds, name =xnam))  
    return df      
...
　  随机指标一般是根据统计学的原理，通过一个特定的周期（常为9日、9周等）内出现过的最高价、最低价
  ...
  然后根据平滑移动平均线的方法来计算K值、D值与J值，并绘成曲线图来研判股票走势。
    ...
    ds=SOk.ewm(span = n).mean()
    SOd = pd.Series(ds, name = xnam)
...
　　经过一连串参数顺化的过程后，UOS指标比一般单一参数的振荡指标，更能够顺应各种不同的市况。
    ...
    while i <  len(df) - 1:   #df.index[-1]:  
        ...
        i = i + 1  
    #
    ds7=pd.Series(BP_l).rolling(window=7,center=False).sum()
    ds14=pd.Series(BP_l).rolling(window=14,center=False).sum()
    ds28=pd.Series(BP_l).rolling(window=28,center=False).sum()
    ds7x=pd.Series(TR_l).rolling(window=7,center=False).sum()
    ds14x=pd.Series(TR_l).rolling(window=14,center=False).sum()
    ds28x=pd.Series(TR_l).rolling(window=28,center=False).sum()
    UltO = pd.Series(4*ds7/ds7x+2*ds14/ds14x+ds28/ds28x, name =xnam) 
    ...
    #    + (pd.rolling_sum(pd.Series(BP_l), 28) / pd.rolling_sum(pd.Series(TR_l), 28)), name =xnam)  # 'Ultimate_Osc'
    #df = df.join(UltO)      
    UltO.index=df.index;
    df[xnam]=UltO
    return df
...
def VORTEX(df, n):
    ...
    while i < len(df) - 1:  # df.index[-1]:
        ...
        i = i + 1
    #
    dsVM=VM.rolling(window=n,center=False).sum()
    dsTR=TR.rolling(window=n,center=False).sum()
    ds = pd.Series(dsVM/dsTR, name=xnam)
